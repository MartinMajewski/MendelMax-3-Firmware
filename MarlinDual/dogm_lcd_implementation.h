...
static void lcd_implementation_status_screen()
{
 ...
 // 1 Extruder (Only One Extruder, so it is the left most extruder with ID 0
 #if EXTRUDERS == 1
 u8g.setFont(FONT_STATUSMENU);
 u8g.setPrintPos(6,6);
 ...
 // 2 Extruders (Exactly two extruders, so the first one (ID 0) is right and the second one (ID 1) ist left
 #if EXTRUDERS == 2
 // Draw second extruder leftmost
 u8g.setFont(FONT_STATUSMENU);
 u8g.setPrintPos(6,6);
 ...
 // Draw primary extruder rightmost
 u8g.setFont(FONT_STATUSMENU); 
